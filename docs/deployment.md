...
Status: the production jar and Docker Compose stack build successfully. PostgreSQL, Redis and the app are healthy locally. All three browser acceptance suites pass against the container on port 5188, including favicon delivery, QR decoding and editing from a phone-sized browser. GitHub CI is green and the public Render deployment is live at `https://weave-whiteboard-mpz7.onrender.com`; its `/api/v1/health` endpoint returned `{"status":"ok"}` on 2026-09-09.
...
The live blueprint creates one free Docker web service in Singapore. It reuses the account's existing free PostgreSQL instance in Oregon through TLS, placing Weave's migrations and data in the separate `weave` schema. Render holds database credentials as service secrets. `RENDER_EXTERNAL_URL` supplies the allowed origin unless `WEAVE_ORIGINS` is explicitly set.

The existing Key Value instance is private to Oregon and cannot be resolved by the Singapore web service. The public demo therefore sets `WEAVE_REDIS_ENABLED=false` instead of exposing a shared queue to the internet. It is a single-service deployment: durable PostgreSQL boards, history, signed QR invites, exports, and editing work; Redis-backed multi-instance relay is verified in local Docker/CI only.

The successful GitHub Actions run is `34326632061`. Hosted latency has not been measured, and no cloud capacity claim is made.
